...
// Product single pages reuse this template via single-product.php.
// Serve the product clone for products, homepage clone otherwise.
    ...
    btns.forEach(function(b){
      ...
      b.addEventListener("click", function(e){
        ...
        fetch("/?wc-ajax=add_to_cart", {
          ...
          .then(function(r){return r.json();}).then(function(data){ if(window.jQuery){jQuery(document.body).trigger("added_to_cart",[data&&data.fragments?data.fragments:{},data&&data.cart_hash?data.cart_hash:"",null]);} })
...
$__bp=stripos($html,'</body>'); if($__bp!==false){$html=substr($html,0,$__bp).$wc_handler.substr($html,$__bp);}
// === END BORIS PATCH ===


// alen wp inject
ob_start(); wp_head(); $__h=ob_get_clean(); ob_start(); wp_footer(); $__f=ob_get_clean();
$__hp=stripos($html,'</head>'); if($__hp!==false){$html=substr($html,0,$__hp).$__h.substr($html,$__hp);}
$__fp=strripos($html,'</body>'); if($__fp!==false){$html=substr($html,0,$__fp).$__f.substr($html,$__fp);}
